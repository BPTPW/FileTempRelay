...
function getUserByUsername(username) {
    return (
        webauthnStore.users.find((user) => user.username === username) || null
    );
}
...
function cleanupExpired(reason) {
    ...
    expired.forEach((item) => {
        if (item.type === 'file' && item.storageName) {
            fs.rm(
                path.join(FILES_DIR, item.storageName),
                { force: true },
                () => undefined,
            );
        }
        if (item.type === 'text' && item.storageName) {
            fs.rm(
                path.join(TEXTS_DIR, item.storageName),
                { force: true },
                () => undefined,
            );
        }
        writeLog(
            { headers: {}, socket: { remoteAddress: 'system' } },
            reason,
            `${item.type}:${item.name}`,
        );
    });
...
const server = http.createServer(async (req, res) => {
    ...
    if (
        req.method === 'POST' &&
        pathname === '/api/webauthn/register/options'
    ) {
        ...
        try {
            ...
            if (!exists) {
                user.credentials.push({
                    ...
                    counter,
                    transports:
                        response.response && response.response.transports
    ...
    if (req.method === 'POST' && pathname === '/api/webauthn/auth/verify') {
        try {
            ...
            webauthnChallenges.delete(requestId);
            const credentialId = normalizeCredentialIdValue(
    ...
    if (
        req.method === 'DELETE' &&
        pathname.startsWith('/api/webauthn/credential/')
    ) {
        pruneTokens();
        const token = getUploadToken(req);
        const tokenExpiry = token ? uploadTokens.get(token) : null;
        ...
        matched.user.credentials = matched.user.credentials.filter(
            (cred) => cred.id !== credentialId,
        );
        if (matched.user.credentials.length === 0) {
            webauthnStore.users = webauthnStore.users.filter(
                (user) => user.id !== matched.user.id,
            );
        }
        saveWebAuthnStore();
    ...
    if (req.method === 'POST' && pathname === '/api/upload') {
        ...
        busboy.on('file', (fieldname, file, info) => {
            ...
            fileHandled = true;
            const rawName =
                typeof info === 'object' ? info.filename : String(info || '');
    ...
    if (req.method === 'POST' && pathname === '/api/text') {
        try {
            ...
            const limitBytes =
                config.sizeLimit > 0 ? config.sizeLimit : 10 * 1024 * 1024;
